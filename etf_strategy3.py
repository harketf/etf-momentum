...
"""
ETF动量轮动策略 - 3号纳创恒油策略
ETF池：纳指ETF(159941)、创业板100ETF(159915)、石油LOF(162719)、恒生ETF(159920)
...
FALLBACK_ETF = {'518880': '黄金ETF'}  # 空仓时备选

MOMENTUM_DAYS = 20       # 动量周期（实际20交易日）
HOLD_DAYS = 5            # 最小持有天数
THRESHOLD_MIN = -0.03    # 动量阈值下限
...
def calculate_momentum(symbol):
    ...
    all_names = {**ETF_POOL, **FALLBACK_ETF}
    return {
        'symbol': symbol,
...
def generate_signal():
    ...
    # 没有ETF在阈值内，买入备选黄金ETF
    if best_etf is None:
        fallback_symbol = list(FALLBACK_ETF.keys())[0]
        fallback_data = calculate_momentum(fallback_symbol)
        if fallback_data:
            best_etf = fallback_data
            reason_prefix = "无ETF在阈值内，买入备选黄金ETF"
        else:
...
def format_email(signal):
    ...
    html += f"""
    <h3>持仓状态</h3>
    <p><strong>当前持仓:</strong> {all_names.get(hold, '无持仓')} {f'({hold})' if hold else ''}</p>
    ...
    <p style="color:gray;font-size:12px;">
        策略参数: 动量周期={MOMENTUM_DAYS}交易日 | 最小持有={HOLD_DAYS}交易日 | 阈值=[{THRESHOLD_MIN:.1%}, {THRESHOLD_MAX:.1%}] | 备选={list(FALLBACK_ETF.values())[0]}
    </p>
    """
    return html
...
def main():
    print(f"\n{'='*50}")
    print(f"3号纳创恒油策略执行 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*50}\n")
    ...
    if error:
        subject = f"[3号策略错误] {datetime.now().strftime('%m-%d')}"
        content = f"<h2>策略执行失败</h2><p>{error}</p>"
    else:
        content = format_email(signal)
        action = "买入" if signal['should_trade'] else "持有"
        subject = f"[3号{action}] {signal['best_etf']['name']} | 动量{signal['best_etf']['momentum']:.1%} | {datetime.now().strftime('%m-%d')}"
